# Earnings Analysis: AMZN — Q2 2026

*Analysis date: 2026-08-06 | Report date: 2026-07-30 | Price: $271.58*

## Summary

| Metric | Value |
|--------|-------|
| Weighted Fair Value | **$289.45** |
| Upside from Current | +6.58% |
| Consensus PT | $322 |
| Verdict | Below consensus ($322) by 10.11% — more cautious |

## ⚠️ Income Quality Adjustment

Headline EPS $5.75 includes one-time items:

- **Anthropic investment mark-to-market gain**: $53.4B (non-operating) — $13B cost basis revalued to $190.4B; unrealized, non-cash. Inflates headline EPS from ~$1.87 to $5.75

Operating EPS (adjusted): **$1.87**

## Earnings Breakdown

| Metric | Actual | Estimate | Beat/Miss |
|--------|--------|----------|-----------|
| Revenue | $200.6B | $196.16B | ✅ Beat |
| Op. Income | $27.5B | — | +43% YoY |
| EPS (operating) | $1.87 | $1.81 | ✅ Beat |

### Segments

| Segment | Revenue | Growth | Op. Margin |
|---------|---------|--------|------------|
| AWS | $42.2B | +36.7% | 36.8% |
| North America | $116.2B | +16% | 7.9% |
| International | $42.2B | +15% | 4% |
| Advertising | $19.8B | +26% | — |

## Catalysts

| # | Catalyst | Dir. | Prob. | Horizon | Anchor |
|---|----------|------|-------|---------|--------|
| 1 | AWS growth sustains >30% through 2027 | 📈 growth | 65% | medium-term | $496B backlog provides ~3yr revenue coverage at current ARR; 5th consecutive quarter of acceleration; demand exceeds capacity through 2027 per Jassy |
| 2 | Advertising becomes second high-margin pillar ($80B+ run rate) | 📈 growth | 70% | near-term | Ad revenue at $19.8B/quarter (+26% YoY) = $79B ARR already; historically high-margin (est. 50%+); powered by AI tools and streaming expansion |
| 3 | Fed rate cuts in late 2026/2027 expand tech multiples | 📈 growth | 40% | medium-term | CME FedWatch pricing cuts pushed to late 2026-2027; BIS notes AI capex complicating rate decisions; lower rates historically expand growth stock P/Es by 2-4x turns |
| 4 | FCF stays negative through mid-2027 as capex hits $220B+ | 📉 risk | 60% | near-term | TTM FCF already -$7.6B; capex raised twice in 6 months ($200B→$220B); memory cost inflation driving unplanned increases; 6 consecutive quarters of FCF decline |
| 5 | Open-source models (Llama 4, DeepSeek V4) commoditize AI inference, reducing cloud pricing power | 📉 risk | 35% | medium-term | DeepSeek V4 matches GPT-4o on 7/12 benchmarks; Llama 4 Maverick at GPT-5.3 level; self-hosting reduces inference costs 60-80% vs API pricing; but frontier reasoning (Claude, o-series) retains premium for complex enterprise tasks |
| 6 | FTC antitrust trial (Oct 2026) produces headline risk or structural remedy | 📉 risk | 25% | near-term | Trial confirmed for Q4 2026; historically FTC wins in tech antitrust are rare (<30%); remedies lag verdicts by years; but German cartel precedent on algorithmic pricing established new standard |
| 7 | AI investment cycle correction — hyperscaler capex questioned, sector de-rating | 📉 risk | 30% | medium-term | BIS: 5 largest hyperscalers spending >$1T combined in 2025-2026; FactSet scenario analysis places correction stress-test at 2027-2028; historical analog: 2021-22 cloud capex surge preceded 2022-23 optimization cycle with 12-18mo lag |
| 8 | Anthropic valuation sustains at $965B+ (supporting Amazon's $190B mark) | 📈 growth | 50% | medium-term | Anthropic at $965B valuation; Claude maintains reasoning advantage; but a reversal would hit sentiment even though non-operating. IPO reportedly being considered — would crystallize or challenge the mark. |

## 🏰 Economic Moat & Competitor Benchmarking

- **Overall Moat Rating:** **Wide Moat**
- **Moat Trend:** **Widening**

### Moat Sources

| Moat Source | Strength | Durability | Description |
|-------------|----------|------------|-------------|
| Network Effects | Strong | 15 yrs | Two-sided e-commerce marketplace flywheel with over 300M active customer accounts and millions of third-party sellers. Prime member retention (>95% annual renewal) and FBA fulfillment integration create self-reinforcing liquidity. |
| Cost Advantage | Strong | 15 yrs | Massive regionalized fulfillment network with automated robotics (Proteus/Sparrow), internal line-haul transport, and custom hyperscale silicon (Trainium, Inferentia, Graviton) driving superior unit economics in compute and logistics. |
| Switching Costs | Strong | 12 yrs | Enterprise cloud workloads on AWS feature deep architectural entrenchment across proprietary databases (Aurora, DynamoDB), security IAM, Bedrock foundation models, and multi-year Enterprise Discount Programs (EDP). |
| Intangible Assets | Strong | 15 yrs | Top-tier global brand equity and default starting point for over 60% of US commercial product searches, supporting a high-margin retail media advertising platform ($19.8B/quarter, +26% YoY). |
| Efficient Scale | Moderate | 10 yrs | Sub-same-day delivery infrastructure and regional multi-tier fulfillment hubs represent multi-hundred-billion-dollar cumulative Capex investments that deter rational greenfield replication. |

### Competitor Peer Benchmarking

| Peer | Market Cap | Revenue | YoY Growth | Gross Margin | Op. Margin | Forward P/E | Market Share | Pricing Power | Key Advantage / Vulnerability |
|------|------------|---------|------------|--------------|------------|-------------|--------------|---------------|-------------------------------|
| **MSFT** (Microsoft Corporation) | $3350B | $245B | +15% | 70% | 45% | 31x | 24% | Superior | Advantage: Deep enterprise software lock-in with Office/Windows and OpenAI model access. Vulnerability: Higher reliance on external GPU capacity and enterprise software margin compression. |
| **GOOGL** (Alphabet Inc.) | $2250B | $350B | +14% | 57% | 32% | 21.5x | 11% | Superior | Advantage: Dominant consumer internet search and in-house TPU silicon leadership. Vulnerability: Trailing #3 position in enterprise cloud infrastructure and vulnerability of core search to AI agents. |
| **WMT** (Walmart Inc.) | $620B | $685B | +5.5% | 24.5% | 4.5% | 28x | 12% | Parity | Advantage: Unmatched physical store proximity for grocery and immediate pickup. Vulnerability: Lower digital e-commerce profitability and zero high-margin cloud infrastructure offset. |
| **BABA** (Alibaba Group) | $210B | $135B | +6% | 38% | 15% | 9.5x | 14% | Inferior | Advantage: Massive scale across Chinese domestic retail supply chains and cross-border manufacturing. Vulnerability: Geopolitical headwinds, Chinese consumer demand slowdown, and intensifying domestic competition from PDD/Douyin. |
| **SHOP** (Shopify Inc.) | $105B | $8.5B | +21% | 50% | 12% | 55x | 3% | Parity | Advantage: Preferred platform for direct-to-consumer brand identity and autonomy. Vulnerability: Relies on third-party carrier logistics and lacks integrated hyperscale cloud/ad ecosystems. |

### Competitive Dynamics Summary

> Amazon operates a formidable multi-engine ecosystem spanning global e-commerce, third-party seller logistics (FBA), high-margin retail media advertising, and hyperscale cloud infrastructure (AWS). AWS growth has re-accelerated to +36.7% YoY, capturing significant incremental enterprise generative AI workloads through Amazon Bedrock and custom Trainium2/Inferentia2 silicon. Advertising revenue continues to expand at +26% YoY with industry-leading ROAS driven by high-intent shopping queries. The integration of robotics across North American regional fulfillment networks has driven structural cost-to-serve reductions, cementing Amazon's Wide Moat across both commercial cloud and physical distribution.

## ⚡ StressAlpha Dynamic Valuation Bands & Flow-Through

- **Stressed Diluted EPS:** $10.5
- **Stressed Revenue:** $820B
- **Stressed Gross Profit:** $397.7B
- **Stressed Operating Income:** $132.7B
- **Stressed Net Income:** $110.8B
- **Downside to Panic Floor:** -30.41%
- **Market Priced-In Multiple:** 25.86x

| Regime | Multiple | Target Price | Delta vs Current |
|--------|----------|--------------|------------------|
| 🐂 Bull | 32x | $336 | +23.72% |
| ⚖️ Base | 28x | $294 | +8.26% |
| 🚨 Panic | 18x | $189 | -30.41% |

## Scenario Analysis

| Scenario | Prob. | Fair Value | Upside |
|----------|-------|------------|--------|
| Bull | 30% | $368 | +35.5% |
| Base | 45% | $294 | +8.26% |
| Bear | 25% | $187 | -31.14% |

## Sensitivity Analysis

| Scenario | Parameter | Base → Alt | FV Delta |
|----------|-----------|------------|----------|
| Bull | EPS +10% | 11.5 → 12.65 | +$36.8 |
| Bull | EPS -10% | 11.5 → 10.35 | $-36.8 |
| Bull | Multiple +2 | 32 → 34 | +$23 |
| Bull | Multiple -2 | 32 → 30 | $-23 |
| Base | EPS +10% | 10.5 → 11.55 | +$29.4 |
| Base | EPS -10% | 10.5 → 9.45 | $-29.4 |
| Base | Multiple +2 | 28 → 30 | +$21 |
| Base | Multiple -2 | 28 → 26 | $-21 |
| Bear | EPS +10% | 8.5 → 9.35 | +$18.7 |
| Bear | EPS -10% | 8.5 → 7.65 | $-18.7 |
| Bear | Multiple +2 | 22 → 24 | +$17 |
| Bear | Multiple -2 | 22 → 20 | $-17 |

## Historical Reactions

> The market's revealed rule in 2026: capex is punished when growth is ambiguous or unquantified (Feb: -18%), rewarded when AWS growth validates the spend (Jul: +15%). The key variable for next earnings (Oct) is whether AWS sustains >35% AND whether management provides granular ROI data. FTC trial in the same month compounds event risk. Historical pattern: pre-earnings anxiety creates the setup for post-earnings relief — but only if the numbers validate.

| Date | Event | Price Move | Context |
|------|-------|------------|---------|
| 2026-07-30 | Q2 2026 earnings: AWS +37%, capex raised to $220B, Jassy 'clear line of sight' to ROI | +15.3% | Market rewarded because AWS growth validated the capex thesis. Stock had pulled back 11% in prior weeks on capex anxiety, creating setup for relief rally. |
| 2026-02-23 | $200B capex announcement with no ROI specificity | -18% | Longest daily losing streak in 20 years. Market punished capex without growth validation. $460B in market value erased in 2 weeks. |
| 2026-06-23 | Record Prime Day ($26.3B ecosystem spending) but stock fell on broader AI capex rotation | -5% | Strong consumer demand couldn't offset sector rotation out of AI capex names. Market cared more about capex narrative than retail fundamentals. |
| 2026-07-14 | Pre-earnings pullback: stock at $240, down from $278 high | -11% | Market anxiously positioning ahead of capex guidance. FCF tension was the dominant narrative. Set up the 15% post-earnings rally. |
